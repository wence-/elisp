...
;; mp3player.el <URL: http://www.nongnu.org/mp3player/>.
...
available), COMMENT is decoded into utf-8."
  ...
  (list (car comment)
        (oggc-decode-utf-8 (cadr comment))))
...
(defun oggc-valid-ogg-stream-p ()
  "Return T if the current buffer contains a valid Ogg-Vorbis stream."
  (assert (search-forward oggc-ogg-header (min 100 (point-max)) t)
          nil "File does not appear to be a valid ogg stream")
...
COMMENTS should be as for `oggc-construct-comment-string' (q.v.)."
  (with-temp-buffer
    ...
    (assert (oggc-valid-ogg-stream-p))
...
(defun oggc-read-vendor ()
  "Read and Ogg-Vorbis vendor string from the current buffer."
  (let ((length (oggc-bytes-to-lsb-int 4)))
    (oggc-read-string length)))

(defun oggc-read-comments (pos)
  "Read Ogg-Vorbis comments from `current-buffer', starting POS bytes from `point-min'."
  (goto-char pos)
...
(defun oggc-read-header (file)
  ...
  (oggc-with-part-of-file (file 0
                                ;; Lets hope that the comments
                                ;; aren't more than 28KB long.
                                (* 1024 28))
                          (assert (oggc-valid-ogg-stream-p))
                          (oggc-binding-if (pos (oggc-comment-exists-p))
                                           (oggc-read-comments pos))))
...
(defmacro* oggc-with-part-of-file ((file beg end) &rest body)
  "Execute BODY in a buffer containing part of FILE.

BEG and END are as `insert-file-contents' (q.v.).

\(fn (FILE BEG END) &rest BODY)"
  `(with-temp-buffer
     (insert-file-contents-literally ,file nil ,beg ,end)
     (goto-char (point-min))
     ,@body))

(defmacro* oggc-binding-if ((val test-form) then &rest else)
  "Like `if', but with VAL bound to the result of TEST-FORM.
VAL is accessible in the THEN and ELSE clauses.

\(fn (VAL TEST-FORM) THEN &rest ELSE)"
  `(let ((,val ,test-form))
     (if ,val
         ,then
         ,@else)))
